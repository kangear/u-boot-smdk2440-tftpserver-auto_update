	...
	for (;
		...
		if (!opts->quiet) {
			...
			if (percent != percent_complete) {
				...
				printf("\rErasing at "FMT64X" -- %3d%% complete.",
				       ARG64(erase.addr), percent);

				if (opts->jffs2 && result == 0)
					printf(" Cleanmarker written at "FMT64X".",
					       ARG64(erase.addr));
			}
		}
	}
...
/*^@@+17@@<8>*/
    ...
    if (nand->rw_oob == 1) {
        ...
        if ((*length)%(oobsiz + datsiz) != 0) {
            printf("Yaffs_write need align on %d + %d bytes boundary!\n",
                   oobsiz, datsiz);
            *length = 0;
            return -EINVAL;
        }
...
/*$*/
	...
	while (left_to_write > 0) {
		...
		if (nand_block_isbad (nand, offset & ~(nand->erasesize - 1))) {
			printf ("Skip bad block "FMT64X"\n",
				ARG64(offset & ~(nand->erasesize - 1)));
		       ...
		       u_char *buffer)
{
	...
	if (len_incl_bad == *length) {
		...
		if (!rval || rval == -EUCLEAN)
			return 0;
		printf ("NAND read from offset "FMT64X" failed %d\n",
			ARG64(offset), rval);
		return rval;
	}
	...
	while (left_to_read > 0) {
		...
		if (nand_block_isbad (nand, offset & ~(nand->erasesize - 1))) {
			printf ("Skipping bad block "FMT64X"\n",
				ARG64(offset & ~(nand->erasesize - 1)));
			offset += nand->erasesize - block_offset;
			continue;
		}
